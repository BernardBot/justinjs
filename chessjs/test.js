...
const epd = `rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 ;D1 20 ;D2 400 ;D3 8902 ;D4 197281 ;D5 4865609 ;D6 119060324
...
// takes about 1 minute on my M1 macbook air
function test_epd(max_moves=5e6) {
	let t = Date.now()
	let total_moves = 0;
	for (let test of epd.split("\n")) {
		const split = test.split(";");
		const fen = split[0];
		parseFen(fen);
		for (let depth_test of split.slice(1)) {
			const depth_split = depth_test.split(" ");
			const depth = parseInt(depth_split[0].slice(1));
			const expected = parseInt(depth_split[1]);
			if (expected > max_moves) {
				continue;
			}
			const actual = perft(depth);
			if (expected != actual) {
				console.error("FAILED", fen, depth, expected, actual);
			} else {
				console.log("SUCCES", fen, depth, expected, actual);
			}
			total_moves += actual;
		}
	}
	const dt = Date.now() - t;
	console.log("dt", "total_moves", "total_moves / dt");
	console.log(dt, total_moves, total_moves / dt);
}
